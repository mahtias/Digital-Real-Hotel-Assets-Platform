...
export const yieldService = {
  ...
  async distributeFromBooking(
    ...
    // Inner logic — runs with whatever db client is passed
    const runInDb = async (db: any) => {
      ...
      if (investments.length === 0) {
        return {
          ...
          totalInvested: 0,
          investors: 0,
          batchDistributionIds: [] as string[],
          batchInvestors: [] as `0x${string}`[],
          batchAmounts: [] as bigint[],
          yieldRowsToUpdate: [] as string[],
        };
      }

      // 5. CALCULATE TOTAL INVESTED
      const totalInvested = investments.reduce(
        (sum: number, inv: any) => sum + Number(inv.investedAmount),
        0
      );
      if (totalInvested <= 0) throw new Error("Total invested amount is zero");
      ...
      for (const inv of investments) {
        ...
        batchInvestors.push(inv.user.walletAddress as `0x${string}`);
        batchAmounts.push(
          BigInt(
            Math.round(
              investorYield * (10 ** (STABLECOIN_REGISTRY[stablecoinSymbol]?.decimals ?? 6))
            )
          )
        );
    ...
    // If already inside a prisma.$transaction, run directly (no nesting allowed)
    const result = tx
      ? await runInDb(tx)
      : await prisma.$transaction(runInDb);
    ...
    // 8. ALLOCATE ON CHAIN
    try {
      const txResult = await hotelYieldVaultService.addClaimableBatch(
      ...
      if (!tx) {
        // Standalone call (no outer transaction) — mark records as FAILED explicitly.
        // When tx IS set, the outer transaction rolls back automatically on rethrow.
        await prisma.yield_distributions.update({
          where: { id: result.distribution.id },
          data: { status: "FAILED" },
        }).catch(() => {});
  ...
  async distributePendingYields() {
    ...
    const bookings = await prisma.booking.findMany({
      where: {
        status: "PAID",
        yield_distributions: { none: {} }, // skip already-distributed bookings
      },
    });
